  ...
  try {
    ...
    if (!planName && subscription.items.data.length > 0) {
      ...
      if (planFromPrice) {
        ...
        console.log(
          "⚠️ Unknown price ID:",
          priceId,
          "- not found in subscription_plans table",
        );
      }
    }
    ...
    if (subscriptionError) {
      ...
      if (subscriptionError.code === "23505") {
        console.log(
          "ℹ️ Subscription already exists, continuing to ensure usage record exists",
        );
      } else {
    ...
    if (isTrialing && trialEnd) {
      ...
      if (!existingUsage) {
        ...
        if (usageError) {
          ...
          console.log(
            "✅ Trial usage record created with period:",
    ...
    if (isTrialing) {
      console.log(
        "📝 Creating billing record for trial subscription, userId:",
        userId,
      );
      ...
      if (oldRecords && oldRecords.length > 0) {
        console.log(
          "🧹 Cleaning up",
          oldRecords.length,
          "old billing records for new trial",
        );
  ...
  try {
    ...
    if (isTrialEnding) {
      ...
      if (insertError) {
        ...
        console.log(
          "✅ Usage reset to 0 (created new record for period:",
          periodStart,
          "to",
          periodEnd,
          ")",
    ...
    if (profileUpdateError) {
      console.error(
        "❌ Error updating profile in subscription update:",
        profileUpdateError,
      );
    } else {
      console.log(
    ...
    if (oldPlan !== newPlan) {
      ...
      if (profile && newPlanData) {
        ...
        console.log(
          `✅ ${isUpgrade ? "Upgrade" : "Downgrade"} email sent to:`,
          profile.email,
        );
      }
    }
...
async function handleInvoicePaymentSucceeded(
  ...
  try {
    ...
    if (invoice.amount_paid === 0) {
      console.log(
        "ℹ️ Invoice amount is $0 (trial period), skipping - no payment made",
      );
      return;
    }
    ...
    if (!userId && invoice.customer) {
      ...
      if (subFromCustomer) {
        ...
        if (subFromCustomer.stripe_subscription_id && !subscription) {
          try {
            subscription = await stripe.subscriptions.retrieve(
              subFromCustomer.stripe_subscription_id,
            );
          } catch (e) {
            console.log(
              "⚠️ Could not retrieve subscription from Stripe, continuing...",
            );
          }
        }
      }
    ...
    if (!userId) {
      console.error(
        "❌ Could not determine userId from subscription or customer",
      );
      return;
    }
    ...
    if (pendingRecords && pendingRecords.length > 0) {
      ...
      const subscriptionStartRecord = pendingRecords.find(
        (r: any) => r.action === "subscription_start",
      );
      ...
      const planChangeRecords = pendingRecords.filter(
        (r: any) => r.action === "upgrade" || r.action === "downgrade",
      );
      ...
      console.log(
        ...
        "pending billing records to paid",
      );
