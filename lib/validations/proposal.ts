...
// Relaxed to match current UI fields (operating_hours, cleaning_schedule_preference, security_requirements)
// Optional fields kept for future expansion without blocking navigation
export const commercialServiceSchema = z.object({
  operating_hours: z.string().min(1, 'Operating hours are required'),
  cleaning_schedule_preference: z.enum([
    'before_hours',
    'after_hours',
    'during_hours',
  ]),
  security_requirements: z.string().optional(),
  // Optional/unrendered fields
...
export const windowServiceSchema = z.object({
  ...
  window_types: z
    .array(z.enum(['standard', 'french', 'bay', 'skylight', 'storm']))
    .default([]),
...
// Relaxed to match current UI fields (floor_condition, drying_time_preference, furniture_moving)
export const floorServiceSchema = z.object({
  ...
  floor_types: z
    .array(
      z.enum(['hardwood', 'tile', 'laminate', 'vinyl', 'concrete', 'marble'])
    )
    .default([]),
  treatment_needed: z
    .array(z.enum(['deep_clean', 'strip_wax', 'refinish', 'seal', 'polish']))
    .default([]),
  high_traffic_areas: z.array(z.string()).default([]),
});
...
export const validateProposalWithServiceData = (data: any) => {
  ...
  const serviceSpecificValidation = serviceSpecificSchema.parse(
...
export const pricingSettingsSchema = z.object({
  ...
  overhead_percentage: z
    .number()
    .min(0)
    .max(100, 'Overhead percentage must be between 0-100'),
  margin_percentage: z
    .number()
    .min(0)
    .max(100, 'Margin percentage must be between 0-100'),
