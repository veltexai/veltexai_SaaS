...
## Independent result and small follow-up

Claude's `CLAUDE_ROUND4_REVIEW_RESULT.md` gives candidate `dde3c0c` a local PASS and release FAIL. It executed all 45 committed migrations without scratch patches using the derived harness, cross-checked its original harness, passed ownership/privacy/statistics/service-role/dirty-event/repeatability checks, and counted 40 concurrent views exactly. The mandatory definer gate independently failed on 10 legacy functions, as expected. This closes the migration-syntax uncertainty on the repository schema, not the real-target/Supabase/staging gates.

The final follow-up addresses R4-1 with a prominent operator warning whenever initial-clean price is below the ongoing visit, without silently changing prices or customer prose. It also addresses R4-2 with explicit marker/connection errors and unconditional refusal even when sourced without `set -e`. Local tests: 506 tests/57 suites/5 snapshots, typecheck and production build pass; an unmarked directory exits 2 with a clear message and never reaches the command after the guard. No migrations changed after Claude's verified `dde3c0c` chain. R4-3 harmless direct-EXECUTE grants on trigger-returning functions and R4-5 advisory heuristic limitations remain low-priority inventory/UX follow-ups; R4-4 non-owner statistics intentionally reveal no distinction from zero views. R3-3 remains a release blocker with no waiver.
