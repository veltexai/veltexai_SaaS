...
it.each(['one-time', 'one_time'])('shows one-time labels for %s', serviceFrequency => {
  ...
  expect(screen.getByText('Suggested One-Time Price (Base + Add-ons)')).toBeInTheDocument();
...
it('preserves recurring pricing labels', () => {
  ...
  expect(screen.getByText('Suggested Monthly Price (Base + Add-ons)')).toBeInTheDocument();
