...
export function estimateJob(input: unknown) {
  ...
  const modeledHours = (job.squareFeet / service.production.base * 0.65 + job.bedrooms * 0.2 + job.bathrooms * 0.5 + ((job.levels ?? 1) - 1) * 0.2 + (job.hardFloorPercent ?? 50) / 100 * 0.2 + Math.max(0, (job.occupants ?? 2) - 2) * 0.08 + Math.min(job.monthsSinceClean ?? 1, 12) * 0.04) * condition * ({ light: 0.95, normal: 1, heavy: 1.25 }[job.clutter ?? 'normal']) * recurrence + extraHours;
  ...
  const scenario = (factor: number) => {
    ...
    const suggestedPrice = money(Math.ceil(Math.max(c.minimumCharge, beforeMinimum) / (c.roundingIncrement ?? 5)) * (c.roundingIncrement ?? 5));
  ...
  const visitsPerMonth = job.turnover ? (job.turnover.expectedTurns ?? 4) : { 'one-time': 1, weekly: 52 / 12, 'bi-weekly': 26 / 12, '1x-month': 1 }[job.frequency];
  const warnings = ['Planning assumptions have not been validated by a cleaning operator. Confirm after a walkthrough.'];
  if (base.elapsedCrewHours > 8) warnings.push('Estimated visit exceeds eight hours. Agree a larger crew or multiple service days.');
  if (selectedPrice < c.minimumCharge) warnings.push('Your override is below the minimum visit charge.');
  ...
  if (job.turnover) {
    ...
    const windowHours = (minutes(job.turnover.checkin) - minutes(job.turnover.checkout) + (job.turnover.nextDay ? 1440 : 0)) / 60;
