...
export const CATALOG_VERSION = '2026-09-22.1' as const;
...
export const DEFAULT_COSTS: z.infer<typeof costAssumptionsSchema> = {
  crewSize: 2, wage: 25, burdenPercent: 25, supplies: 12, equipment: 5,
...
export const jobSchema = z.object({
  ...
  access: z.string().trim().min(1).max(1000),
  ...
  if (job.turnover && job.turnover.checkin <= job.turnover.checkout)
    ctx.addIssue({ code: 'custom', path: ['turnover', 'checkin'], message: 'Check-in must follow checkout on the same day.' });
...
export const serviceRecordSchema = z.object({
  ...
  questionKeys: z.array(z.string()), strategy: z.literal('residential_labor_v1'),
