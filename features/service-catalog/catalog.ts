...
const common = {
  ...
  strategy: 'residential_labor_v1' as const,
  ...
  terms: 'Suggested price requires operator review and customer acceptance. Confirm service dates, payment timing and cancellation terms in writing before work. Scope changes require a revised agreement. No automatic renewal for one-time jobs.',
};
...
const definitions = [
  ...
  { id: 'first_deep', segment: 'residential', family: 'home_cleaning', label: 'First visit / deep clean', frequencies: ['one-time'], production: { min: 180, base: 280, max: 380 }, inclusions: [...homeScope, 'Detail reachable baseboards, doors and buildup on washable surfaces'] },
  ...
  { id: 'airbnb_turnover', segment: 'short_term_rental', family: 'rental_turnover', label: 'Airbnb / short-term-rental turnover', frequencies: ['one-time'], production: { min: 250, base: 400, max: 550 }, inclusions: [...homeScope, 'Reset beds with available clean linens'] },
];
...
export function getService(id: JobType) {
...
export function defaultJob(id: JobType = 'recurring_standard'): CatalogJob {
  return {
    catalogVersion: CATALOG_VERSION, jobType: id, segment: getService(id).segment,
    frequency: id === 'recurring_standard' ? 'bi-weekly' : 'one-time',
    propertyType: 'house', squareFeet: 1500, bedrooms: 3, bathrooms: 2, occupancy: id === 'move_in_out' ? 'vacant' : 'occupied',
    ...
    ...(id === 'airbnb_turnover' ? { turnover: { checkout: '11:00', checkin: '16:00', laundryLoads: 0, restocking: false, inspection: true, damageDocumentation: true } } : {}),
  };
}
