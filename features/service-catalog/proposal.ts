...
export function composeCatalogProposal(input: unknown): ProposalFormData {
  ...
  const frequency = job.turnover ? 'Per turn' : { 'one-time': 'One-time service', weekly: 'Weekly', 'bi-weekly': 'Every two weeks', '1x-month': 'Monthly' }[job.frequency];
  const scopeRows = inclusions.map(area => ({ area, frequency, notes: '' }));
  const pricingTable = { rows: [{ service: service.label, frequency: frequency, pricePerMonth: usd(estimate.selectedPrice) }], summary: { subtotal: usd(estimate.selectedPrice), tax: 'Not assessed', total: usd(estimate.selectedPrice) } };
  const content = `## Cover letter
${job.coverLetter ? plain(job.coverLetter) : `We propose ${service.label.toLowerCase()} for ${plain(client.client_name)} at ${plain(client.service_location)}.`} Please review the scope and service agreement below.
...
Your price: ${usd(estimate.selectedPrice)} ${job.turnover ? 'per turn' : 'per visit'}.${job.frequency !== 'one-time' || job.turnover ? ` Estimated monthly budget: ${usd(estimate.periodPrice)}; actual billing depends on visits completed. Confirm actual billing schedule in writing.` : ' This is a one-time job total.'}
...
Confirm the scope, scheduling, taxes and payment terms before signing.`;
  return proposalFormSchema.parse({
    ...
    pricing_data: { price_range: { low: estimate.selectedPrice, high: estimate.selectedPrice },
...
export function normalizeCatalogProposal(data: ProposalFormData, existing?: ProposalFormData): ProposalFormData {
  ...
  if (existing && isCatalogProposal(existing)) {
    if (data.service_specific_data.catalogJob.catalogVersion !== existing.service_specific_data.catalogJob.catalogVersion) throw new Error('Create a new proposal to change catalog version.');
  ...
  if (data.service_specific_data.catalogJob.demo) throw new Error('Sample jobs cannot be saved. Start a real job.');
