...
const out = path.join(root, 'quality/service-catalog-release-1/artifacts');
...
for (const type of ['recurring_standard', 'airbnb_turnover']) write(`${type}-workbench`, React.createElement(CatalogWorkbench, { demo: true, initialJobType: type }));
for (const pack of CATALOG) {
  const job = { ...defaultJob(pack.id), access: 'Safe lockbox access; parking beside entrance.' };
