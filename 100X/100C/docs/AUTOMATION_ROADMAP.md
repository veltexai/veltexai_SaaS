...
- **Automatic safety stops** — a non-Draft/Paused campaign, auth/scope/payment/campaign-not-found error,
...
3. Add a **scheduler** (not built here: no cron/route/webhook yet) that invokes the same terminal runner
   on an interval, honoring the lock, caps, and daily/total campaign caps.
4. Build **100D** event ingestion (authenticated, idempotent, append-only, replay-safe) so suppression
...
6. Alerting + reconciliation job to sweep `reconciliation_required` and `failed_retryable` assignments.
