    ...
    const renderListItem = (rawText: string, key: string) => {
      ...
      if (inAbout) {
        return (
          <div
            key={key}
            className="flex items-start gap-3 sm:gap-4 p-4 sm:p-6 md:p-10 pr-4 sm:pr-6 md:pr-8 pb-6 sm:pb-8 md:pb-12"
          >
            {Icon ? (
          ...
          if (inAbout) {
            ...
            elements.push(
              <ul
                ...
                className="mb-2 sm:mb-3 space-y-1 sm:space-y-2"
              >
                {currentList}
              </ul>,
            );
      ...
      <div className="text-white min-w-[280px]">
        {rowChunks.map((chunk, chunkIndex) => (
          <div
            ...
            style={
              chunkIndex > 0
                ? { pageBreakBefore: "always", paddingTop: "20px" }
                : undefined
            }
          >
            ...
            {chunk.map((row, i) =>
              renderRow(row, chunkIndex * MAX_ROWS_PER_SECTION + i, premium),
            )}
          </div>
        ))}
      </div>
...
function AdditionalServicesTable({
  ...
  return (
    <div
      className="my-4 sm:my-6 space-y-2 sm:space-y-3 overflow-x-auto"
      data-extras-ready="true"
    >
